...
class filter(bolflow.bolflow):
    ...
    def filter_freq(self, ftype, fvalue, fcv):
        '''
        Filter by frequency
        '''
        # get dataframe with the frequency columns
        # if QC then get the CV_all column
        logging.debug('get dataframe with the frequency columns')
        fcol = []
        if fcv:
            fcol = [col for col in self.df if col == 'Freq_'+ftype or col == 'CV_all']
        else:
            fcol = [col for col in self.df if col == 'Freq_'+ftype]
        # get the comparative value from the groups and qc using the filter value
        if fcol:
            logging.debug('get the global frequency')
            self.df = self._filter_freq(fcol,fvalue)
