    ...
    def filter(self, ftype, ffreq, fcv):
        ...
        if fcv:
            ...
            if fcol and ccol:
                ...
                self.df = self._filter_freq_cv(ftype,fcol,ffreq,ccol,fcv)
        else:
            # get columns
            fcol = []
            for ft in re.split(r'[|&]', ftype):
            ...
            if fcol:
                ...
                self.df = self._filter_freq(ftype,fcol,ffreq)

    
    def _filter_freq(self, ftype, fcol, fvalue):
        '''
        Filter by frequency
        '''
        df = self.df[fcol]
        ...
        if '&' in ftype:
            d = dc[ dc.any(axis=1) ]
        # OR condition, delete experiments when don't pass all conditions
        else:
            d = dc[ dc.all(axis=1) ]
        df = self.df.drop(d.index.values)
        return df

    def _filter_freq_cv(self, ftype, fcol, ffreq, ccol, fcv):
        '''
        Filter by frequency
        '''
        df_g = self.df[fcol]
